...
public class Cliente {
    ...
    private void historialMensajes(Usuario usuario) {
        ...
        try {
            System.out.println("Solicitando historial de mensajes..."); // Log para depuración

            oos.writeObject(mensaje);
            oos.flush();

            System.out.println("Esperando respuesta del servidor..."); // Log para depuración

            Object respuesta;
            while ((respuesta = ois.readObject()) != null) {
                if (respuesta instanceof List<?>) {
                    ...
                    System.out.println("No envia un mensaje");
                    break;
                }
            }
        } catch (IOException e) {
